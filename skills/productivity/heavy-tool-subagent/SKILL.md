...
- **MCP Subprocess Environment Isolation:** Hermes does NOT automatically pass the host `.env` variables to MCP subprocesses. If you remove an API key from `config.yaml` for security, the MCP server will crash on startup. **Solution:** Create a general bash wrapper script (e.g., `secure-mcp-wrapper.sh`) that extracts the required key from `~/.hermes/.env` without using `source` (to prevent arbitrary code execution), and point the `config.yaml` `command` to this script with empty `env: {}`. Always use the absolute path.
